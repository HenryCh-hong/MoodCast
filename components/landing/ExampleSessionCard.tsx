...
// Shows what makes Moodcast different: the writing quality + Ask DJ interaction
  ...
  return (
    <section className="px-6 py-14 max-w-6xl mx-auto border-t border-mc-border">
      ...
      <div className="flex items-center gap-4 mb-12">
        ...
        <span className="font-mono text-[9px] text-mc-dim whitespace-nowrap">
          no API key required
        </span>
      </div>
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        {SESSIONS.map((s) => (
          ...
          >
            {/* Session meta */}
            <p className="font-mono text-[8px] tracking-[0.12em] uppercase text-mc-lo mb-3 flex items-center justify-between">
              <span>Demo</span>
              <span>{s.length}</span>
            </p>

            <h3 className="text-sm font-semibold text-mc-hi leading-tight mb-1 group-hover:text-mc-lav transition-colors">
              {s.title}
            </h3>
            <p className="font-mono text-[9px] text-mc-lo mb-4">{s.tags}</p>

            {/* Now playing mini */}
            <div className="flex items-center gap-2 pt-3 border-t border-mc-border">
              <span className="font-mono text-[9px] text-mc-lav font-bold">
                {s.nowPlaying.label}
              </span>
              <div className="min-w-0">
                ...
                <p className="font-mono text-[9px] text-mc-lo">{s.nowPlaying.artist}</p>
              </div>
            </div>
          </a>
        ))}
